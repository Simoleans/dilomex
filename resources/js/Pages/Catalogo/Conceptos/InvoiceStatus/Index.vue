    ...
    <AuthenticatedLayout title="Estatus de factura">
        <template #header>
            <div class="flex  gap-4 md:flex-row md:items-center justify-between">
                <h2 class="text-xl font-semibold leading-tight">Estatus de factura</h2>
        ...
        <Modal :show="open" @close="closeModal">
            <div class="p-6">
                <h2 class="text-lg font-medium text-gray-900 dark:text-white">
                    {{ module == 'create' ? 'Crear Estatus de Factura' : (module == 'edit' ? 'Editar Estatus de Factura' : 'Eliminar Estatus de Factura') }}
                </h2>
                <hr class="m-5">
                <form @submit.prevent="handleForm">
                    <div class="mb-3">
                        <InputLabel value="Estatus de Factura:" class="mb-2"/>
...
const handleForm = () => {
    if(module.value == 'create'){
        form.post(route('invoice-status.store'), {
        preserveScroll: true,
        onSuccess: () => {
            toast.add({
        ...
        form.put(route('invoice-status.update', id.value), {
            preserveScroll: true,
            onSuccess: () => {
                toast.add({
                    message: usePage().props.toast.message,
        ...
        form.delete(route('invoice-status.destroy', id.value), {
            preserveScroll: true,
            onSuccess: () => {
                toast.add({
                    message: usePage().props.toast.message,
