<template lang="">
    <AuthenticatedLayout title="Tarifa L. Transporte">
        <template #header>
            <div class="flex  gap-4 md:flex-row md:items-center justify-between">
                <h2 class="text-xl font-semibold leading-tight">Tarifa L. Transporte</h2>
                <button @click="openModal(false,'create')" class="inline-flex items-center px-4 py-2 bg-green-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700 focus:bg-green-700 active:bg-green-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                    ...
                    <span>Crear Tarifa L. Transporte</span>
                </button>
            </div>
        </template>
        ...
        <Modal :show="open" @close="closeModal">
            <div class="p-6">
                <h2 class="text-lg font-medium text-gray-900 dark:text-white">
                    {{ module == 'create' ? 'Crear Tarifa L. Transporte' : (module == 'edit' ? 'Editar Tarifa L. Transporte' : 'Eliminar Tarifa L. Transporte') }}
                </h2>
                <hr class="m-5">
                <form @submit.prevent="handleForm">
                    <div class="mb-3">
                        <InputLabel value="Reparto:" class="mb-2"/>
                        <Dropdown v-model="form.distribution_id" filter :options="distributions" :disabled="module == 'delete'" optionValue="id" optionLabel="name" placeholder="Selecciona un reparto" class="w-full md:w-14rem border-1 border-sky-500" />
                        <InputError class="mt-2" :message="form.errors.distribution_id" />
                    </div>
                    <div class="mb-3">
                        <InputLabel value="Unidad:" class="mb-2"/>
                        <Dropdown v-model="form.uni_type_id" filter :disabled="module == 'delete'" :options="uniTypes" optionValue="id" optionLabel="name" placeholder="Selecciona una unidad" class="w-full md:w-14rem border-1 border-sky-500" />
...
const openModal = (data,type) => {
    ...
    if(type == 'edit'){
        ...
        form.cost = data.cost
        form.distribution_id = data.distribution_id
...
const handleForm = () => {
    if(module.value == 'create'){
        form.post(route('local-transport-rates.store'), {
        preserveScroll: true,
        onSuccess: () => {
            toast.add({
        ...
        form.put(route('local-transport-rates.update', id.value), {
            preserveScroll: true,
            onSuccess: () => {
                toast.add({
                    message: usePage().props.toast.message,
        ...
        form.delete(route('local-transport-rates.destroy', id.value), {
            preserveScroll: true,
            onSuccess: () => {
                toast.add({
                    message: usePage().props.toast.message,
