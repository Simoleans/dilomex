<template lang="">
    <DataTable filterDisplay="row" :value="data" responsiveLayout="scroll" breakpoint="100px" tableStyle="min-width: 10rem" :rowsPerPageOptions="[4,10,20]"  :paginator="true" :rows="8" paginatorTemplate="PrevPageLink PageLinks NextPageLink RowsPerPageDropdown"
    currentPageReportTemplate="Ver {first} a {last} de {totalRecords}" v-model:filters="filters1">
        <template #header>
            <div class="flex md:lg:justify-between flex-row  sm:max-sm:flex-col">
                <Dropdown v-model="filters1['type'].value" :options="options" optionLabel="name" optionValue="value" placeholder="Seleciona un tipo." class="p-column-filter" style="min-width: 2rem" :showClear="true" />
        ...
        <Column field="type" header="Tipo">
            <template #body="{ data }">
                <Tag :value="data.type" severity="primary" v-show="data.type == 1">
                    <span>Chofer</span>
                </Tag>
                <Tag :value="data.type" severity="warning" v-show="data.type == 2">
                    <span>Ayudante</span>
                </Tag>
            </template>
        </Column>
...
const types = ref(['Ayudante','Chofer'])
