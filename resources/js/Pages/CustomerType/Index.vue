<template lang="">
    <AuthenticatedLayout title="Tipos de Clientes">
        <template #header>
            <div class="flex  gap-4 md:flex-row md:items-center justify-between">
                <h2 class="text-xl font-semibold leading-tight">Tipos de Clientes</h2>
        ...
        <Modal :show="open" @close="closeModal">
            <div class="p-6">
                <h2 class="text-lg font-medium text-gray-900 dark:text-white">
                    {{ module == 'create' ? 'Crear Tipo de Cliente' : (module == 'edit' ? 'Editar Tipo de Cliente' : 'Eliminar Tipo de Cliente') }}
                </h2>
                <hr class="m-5">
...
const handleForm = () => {
    if(module.value == 'create'){
        ...
        form.put(route('customer-type.update', id.value), {
            preserveScroll: true,
            onSuccess: () => {
                toast.add({
                    message: usePage().props.toast.message,
        ...
        form.delete(route('customer-type.destroy', id.value), {
            preserveScroll: true,
            onSuccess: () => {
                toast.add({
                    message: usePage().props.toast.message,
