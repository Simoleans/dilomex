<template lang="">
    <AuthenticatedLayout title="Poblacion">
        <template #header>
            <div class="flex  gap-4 md:flex-row md:items-center justify-between">
                <h2 class="text-xl font-semibold leading-tight">Poblacion</h2>
        ...
        <Modal :show="open" @close="closeModal">
            <div class="p-6">
                <h2 class="text-lg font-medium text-gray-900 dark:text-white">
                    {{ module == 'create' ? 'Crear Poblacion' : (module == 'edit' ? 'Editar Poblacion' : 'Eliminar Poblacion') }}
                </h2>
                <hr class="m-5">
...
const handleForm = () => {
    if(module.value == 'create'){
        form.post(route('populations.store'), {
        preserveScroll: true,
        onSuccess: () => {
            toast.add({
        ...
        form.put(route('populations.update', id.value), {
            preserveScroll: true,
            onSuccess: () => {
                toast.add({
                    message: usePage().props.toast.message,
        ...
        form.delete(route('populations.destroy', id.value), {
            preserveScroll: true,
            onSuccess: () => {
                toast.add({
                    message: usePage().props.toast.message,
