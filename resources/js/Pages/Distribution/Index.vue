<template lang="">
    <AuthenticatedLayout title="Repartos">
        <template #header>
            <div class="flex  gap-4 md:flex-row md:items-center justify-between">
                <h2 class="text-xl font-semibold leading-tight">Repartos</h2>
        ...
        <Modal :show="open" @close="closeModal">
            <div class="p-6">
                <h2 class="text-lg font-medium text-gray-900 dark:text-white">
                    {{ module == 'create' ? 'Crear Reparto' : (module == 'edit' ? 'Editar Reparto' : 'Eliminar Reparto') }}
                </h2>
                <hr class="m-5">
...
const handleForm = () => {
    if(module.value == 'create'){
        ...
        form.put(route('distributions.update', id.value), {
            preserveScroll: true,
            onSuccess: () => {
                toast.add({
                    message: usePage().props.toast.message,
        ...
        form.delete(route('distributions.destroy', id.value), {
            preserveScroll: true,
            onSuccess: () => {
                toast.add({
                    message: usePage().props.toast.message,
